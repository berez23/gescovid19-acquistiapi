package it.csi.gescovid.acquistiapi.filter;

import it.csi.gescovid.acquistiapi.business.*;
import it.csi.gescovid.acquistiapi.util.*;
		...
		if (hreq.getSession().getAttribute(IRIDE_ID_SESSIONATTR) == null) {
			...
			if (marker != null) {
				try {
					...
					it.csi.gescovid.acquistiapi.dto.test.UserInfo userInfo = new it.csi.gescovid.acquistiapi.dto.test.UserInfo();
